...
| `SPAWN_GAP` | 135 行 | 出沙口在堆顶上方这么多行（服务端 spawn 位置，客户端据此画水龙头）；配合 0.618 锚点，让水龙头落在视口顶部附近 |
| 重力子步 | 2 / tick | 服务端每 tick 推进 2 次（下落更顺） |
| 镜头锚点 `CAMERA_ANCHOR` | 0.618（黄金分割） | 客户端让堆顶维持在视口从顶往下 **0.618** 处：上方 ~62% 是透明天空 + 固定水龙头，下方 ~38% 是沙塔顶部。堆顶用低通平滑、且按"行内 ≥6 格才算地表"鲁棒测量（`SURFACE_MIN_CELLS`），避免正在下落的沙粒把镜头/水龙头每帧拽得抖动。沙塔够高后镜头跟随上移（`cameraY` 可为负 = 网格上方更多天空） |
| 归档带 `bands`（Stage 3） | 每条 = 1 个世界行（2px 细条） | 活动网格下方堆叠的历史压缩层。客户端把 `worldH = H + bands.length` 作为相机可滚动范围：活动区照旧全分辨率渲染，归档区每条画成一行"每列主色"的细条；向下拖拽即进入历史。收到 `band` 时客户端做与服务端**完全相同**的确定性下移（grid 整体下移 K 行），并把相机/平滑地表同步抬 K，使被跟随的地表视觉不动 |

> 服务端权威下没有客户端"冻结（`FREEZE_AT`/`BAND`）"概念。沉积分两段存：堆顶一段全分辨率留在活动网格里跑物理；很深的老沙由**服务端**压缩成 1px 细条归档（`bands`），客户端只渲染。机制见 `stage3-compression.md` 与 `backend.md`，叙事见 `game-design.md`。
